...
/**
 * Validates models implementing the {@code Rpcv2KestrelTrait} against its constraints by:
 *
 * - Ensuring that every entry in {@code eventStreamHttp} also appears in the {@code http} property
 *   of a protocol trait.
 */
@SmithyInternalApi
public final class Rpcv2KestrelTraitValidator extends AbstractValidator {

    @Override
    public List<ValidationEvent> validate(Model model) {
        List<ValidationEvent> events = new ArrayList<>();
        for (ServiceShape serviceShape : model.getServiceShapesWithTrait(Rpcv2KestrelTrait.class)) {
            Rpcv2KestrelTrait protocolTrait = serviceShape.expectTrait(Rpcv2KestrelTrait.class);

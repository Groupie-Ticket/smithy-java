...
/**
 * A reflective Kestrel payload printer for debugging purposes. Uses a Smithy model to know how to decode
 * Kestrel payloads (instead of seeing them as a bunch of byte lists).
 */
...
public final class Enhancer {
    ...
    /**
     * Dumps a Kestrel-serialized Smithy shape in human-readable format.
     *
     * @param payload a raw Kestrel payload
     * @param shape the shape used to decode this payload. Can be of any type
     */
    public void enhance(byte[] payload, Shape shape) {
        try {
            enhanceNext(new KestrelDeserializer(payload), shape, 0);
        } catch (BadInputException e) {
            printWithIndent(0, "[ERROR (idx: %d)] %s", e.pos, e.getMessage());
        }
    }

    private void enhanceNext(KestrelDeserializer deser, Shape shape, int level) {
    ...
    private void enhanceStruct(KestrelDeserializer deser, StructureShape shape, int level, long len) {
    ...
    private Object getSimpleValue(KestrelDeserializer deser, Shape target) {
    ...
    private String typeSectionTypeToString(KestrelDeserializer deser, int i) {
        return switch (i) {
            case 0 -> "lists";
            case 1 -> "varints";

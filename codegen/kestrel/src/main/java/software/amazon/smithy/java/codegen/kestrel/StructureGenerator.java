...
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.Objects;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.SparseStringList;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.StringList;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.T_EIGHT;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.T_FOUR;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.T_LIST;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.T_VARINT;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.asList;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.byteListLengthEncodedSize;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.decodeElementCount;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.decodeLenPrefixedListLengthChecked;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.encodeByteListLength;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.encodeEightBListLength;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.encodeFourBListLength;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.encodeLenPrefixedListLength;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.encodeVarintListLength;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.intSize;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.lenPrefixedListLengthEncodedSize;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.longSize;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.missingField;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.uintSize;
import static software.amazon.smithy.java.codegen.kestrel.CommonSymbols.ulongSize;
import static software.amazon.smithy.java.codegen.kestrel.Util.isStructure;
...
import software.amazon.smithy.java.codegen.kestrel.CodeSections.EndClassSection;
import software.amazon.smithy.java.codegen.kestrel.CodeSections.StartClassSection;
...
public final class StructureGenerator implements Runnable {
    ...
    private void generateFields(List<MemberShape> fields, int secIdx) {
        ...
        var fieldType = fields.get(0).expectTrait(KestrelFieldTrait.class).getType();
        var required = 0L;
    ...
    private void generateField(MemberShape field, String fieldsetName) {
        ...
        var trait = field.expectTrait(KestrelFieldTrait.class);
        ...
        writer.write(
            "private $T $L;",
            fieldSymbol.getProperty("kestrelField", Symbol.class)
                .orElse(fieldSymbol),
            fieldName
        );
        ...
        writer.openBlock("\npublic $T get$L() {", "}\n", fieldSymbol, methodName, () -> {
            ...
            if (isString(target)) {
                ...
                var kestrelCollectionSymbol = fieldSymbol.expectProperty("kestrelCollection", SymbolReference.class);
                writer.write("""
                    Object field = $L;
                    if (field == null) return null;""", fieldName);
                writer.openBlock("if (field.getClass() == $T.class) {", "}", kestrelCollectionSymbol, () -> {
                    writer.write("""
                        $T m = (($T) field).toMap();
                        this.$L = m;
                        return m;""", fieldSymbol, kestrelCollectionSymbol, fieldName);
                });
    ...
    private void generateSizeMethod() {
        ...
        for (var fieldSet : fieldSets) {
            ...
            if (fieldSet.fieldSetIdx() > 0) {
                writer.writeInline(" + $L", KestrelSerializer.uintSize(fieldSet.fieldSetIdx() - 1));
            }
            writer.writeInline("))");
        }
        writer.write(";");
    ...
    private final class MapSizer extends Sizer {
        ...
        void generate() {
            writer.openBlock("private int $L() {", "}\n", methodName(), () -> {
                ...
                var kestrelCollectionSymbol = mapSymbol.expectProperty("kestrelCollection", SymbolReference.class);
                var valueSymbol = mapValueTarget(model.expectShape(field.getTarget()));
                writer.write("int size;");
                writer.write("if (field.getClass() == $T.class) {", kestrelCollectionSymbol);
                writer.indent().write("size = (($T) field).size();", kestrelCollectionSymbol);
    ...
    private void generateEncoder() {
        generateMethod("public void encodeTo($T s)", CommonSymbols.KestrelSerializer, this::generateEncodeTo);
        if (index.hasVarintMembers(shape)) {
            emitWriteVarints();
        }
                    ...
                    () -> {
                        ...
                        for (int i = 0; i < listFields.size(); i++) {
                            ...
                            if (isString(target)) {
                                ...
                                writer.write(
                                    "(($T) $L).encodeTo(s);",
                                    fieldSymbol.expectProperty("kestrelCollection", SymbolReference.class),
                                    fieldName
                                );
                            } else if (target.isListShape()) {
    ...
    private void generateDecoder() {
        generateMethod("public void decodeFrom($T d)", CommonSymbols.KestrelDeserializer, this::generateDecodeFrom);
        if (index.hasVarintMembers(shape)) {
            ...
            if (varintFieldSetCount > 1) {
                generateMethod(
                    ...
                    CommonSymbols.KestrelDeserializer,
    ...
    private void emitDecoderPrelude(String fieldSetName, String requiredFields, String type) {
        writer.write("""
            KestrelDeserializer.checkFields(fieldSet, $L, "$L");
    ...
    public boolean isRequired(MemberShape field) {
        ...
        return field.expectTrait(KestrelFieldTrait.class).isRequired();
    }

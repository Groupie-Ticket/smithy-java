...
public class KestrelSymbolVisitor implements SymbolProvider, ShapeVisitor<Symbol> {
    ...
    public KestrelSymbolVisitor(Model model, ServiceShape service, KestrelSettings kestrelSettings) {
        ...
        var reservedWords = new ReservedWordsBuilder()
            .loadWords(KestrelSymbolVisitor.class.getResource("reserved-words.txt"), this::escapeWord)
            .build();
    ...
    public Symbol listShape(ListShape shape) {
        ...
        if (memberShape.isStringShape()) {
            b.putProperty("kestrelField", Symbol.builder().name("Object").build());
        }
        return b.build();
    }
    ...
    public Symbol mapShape(MapShape shape) {
        ...
        Class<?> kestrelCollectionImpl = switch (model.expectShape(shape.getValue().getTarget()).getType()) {
        ...
        SymbolReference kestrelCollectionSymbol = Symbol.builder()
            .namespace(kestrelCollectionImpl.getPackageName(), ".")
            .name(kestrelCollectionImpl.getSimpleName())
            .build()
            .toReference(null);
        return mapSymbolBuilder(shape)
            .putProperty("key", keySymbol)
            .putProperty("value", valueSymbol)
            .putProperty("kestrelField", Symbol.builder().name("Object").build())
            .putProperty("kestrelCollection", kestrelCollectionSymbol)
            .addReference(keySymbol)
            .addReference(valueSymbol)
            .addReference(kestrelCollectionSymbol)
            .build();
    }

...
public final class KestrelIndex implements KnowledgeIndex {
    ...
    public KestrelIndex(Model model) {
    ...
    public static KestrelIndex of(Model model) {
        return model.getKnowledge(KestrelIndex.class, KestrelIndex::new);
    }

    private void index(Shape shape, Model model) {
        List<MemberShape> members = new ArrayList<>(shape.members());
        if (members.isEmpty() || !members.stream().findFirst().get().hasTrait(KestrelFieldTrait.class)) {
            return;
        }

        members.sort(
            Comparator.comparing((MemberShape m) -> m.expectTrait(KestrelFieldTrait.class).getType())
                .thenComparing(m -> m.expectTrait(KestrelFieldTrait.class).getFieldSetIdx())
                .thenComparing(m -> m.expectTrait(KestrelFieldTrait.class).getTypeIdx())
        );
        ...
        for (MemberShape ms : members) {
            ...
            KestrelFieldTrait trait = ms.expectTrait(KestrelFieldTrait.class);
    ...
    public boolean hasRequiredLists(ToShapeId shape) {
        return listMembers.getOrDefault(shape.toShapeId(), Collections.emptyMap())
            ...
            .anyMatch(ms -> ms.expectTrait(KestrelFieldTrait.class).isRequired());
    }

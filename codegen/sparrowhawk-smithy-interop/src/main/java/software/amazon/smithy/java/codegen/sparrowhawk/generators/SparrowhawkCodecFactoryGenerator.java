...
public class SparrowhawkCodecFactoryGenerator implements Consumer<JavaWriter> {

    private static final SymbolReference CODEC_FACTORY = imp(SparrowhawkCodecFactory.class);
    private static final SymbolReference SPARROWHAWK_CODEC = imp(SparrowhawkCodec.class);
    ...
    private static final SymbolReference SPARROWHAWK_OBJECT = imp(SparrowhawkObject.class);
    ...
    public SparrowhawkCodecFactoryGenerator(ServiceShape service, Model model, SymbolProvider symbolProvider) {
        this.service = service;
        this.model = model;
        this.symbolProvider = symbolProvider;
        this.className = service.getId().getName() + "SparrowhawkCodecFactory";
    }
    ...
    public void accept(JavaWriter writer) {
        ...
        writer.putContext("sparrowhawkCodec", SPARROWHAWK_CODEC);
        ...
        writer.putContext("sparrowhawkDeser", CommonSymbols.SparrowhawkDeserializer);
        writer.putContext("sparrowhawkSer", CommonSymbols.SparrowhawkSerializer);
        writer.putContext("sparrowhawkObj", CommonSymbols.SparrowhawkObject);
        writer.putContext("sparrowhawkStruct", CommonSymbols.imp(SparrowhawkCodec.class));
    ...
    private record GetCodecGenerator(
        ...
        public void run() {
            writer.write(
                """
                    @Override
                    public ${sparrowhawkCodec:T} getCodec(String operationName) {
    ...
    private static String codecName(String operationName) {
        return operationName + "SparrowhawkCodec";
    }
    ...
    private class CodecGenerator implements Consumer<JavaWriter> {
        ...
        public void accept(JavaWriter writer) {
            for (OperationInfo operation : operations) {
                ...
                writer.putContext("sparrowhawkInput", operation.sparrowhawkInputSymbol);
                writer.putContext("sparrowhawkOutput", operation.sparrowhawkOutputSymbol);
                ...
                writer.putContext("sparrowhawkObject", SPARROWHAWK_OBJECT);
                ...
                var template = """
                    private static final class ${codecClass:L} extends ${sparrowhawkCodec:T} {
                        ...
                        public byte[] encode(${serializableStruct:T} value) {
                            return serialize(${sparrowhawkOutput:T}.convertFrom((${output:T}) value));
                        }
                        ...
                        public ${input:T} decode(${byteBuf:T} buf) {
                            return deserialize(buf, new ${sparrowhawkInput:T}());
                        }
            ...
            public void accept(JavaWriter writer) {
                ...
                writer.putContext("errorMatcher", writer.consumer(w -> {
                    ...
                    for (var error : errors) {
                        ...
                        w.putContext("sparrowhawkError", error);
                        ...
                        w.write("""
                            ${conditional:L} (exception instanceof ${smithyError:T} e) {
                                return ${sparrowhawkError:T}.convertFrom(e);""");
                    }
                    if (!first) {
                        w.write("}\n");
                    }
                ...
                var template = """
                    @Override
                    public ${sparrowhawkObj:T} convertException(Throwable exception) {
                        ${errorMatcher:C|}
        ...
        public void accept(JavaWriter writer) {
            ...
            var decodeTemplate = """
                ...
                public ${input:T} decodeInitialRequest(${byteBuf:T} buffer, ${publisher:T}<?> publisher) {
                    return deserializeStreaming(buffer, new ${sparrowhawkInput:T}(), publisher);
                }""";
            ...
            var sparrowhawkType = symbolProvider.toSymbol(target);
            var smithySymbol = sparrowhawkType.expectProperty(SMITHY_SYMBOL);
            writer.putContext("smithySymbol", smithySymbol);
            writer.putContext("sparrowhawkSymbol", sparrowhawkType);
            writer.write("""

                @Override
                public ${smithySymbol:T} deserializeEvent(${byteBuf:T} buf) {
                    return deserialize(buf, new ${sparrowhawkSymbol:T}());
                }""");
            writer.popState();
        }
    }
        ...
        public void accept(JavaWriter writer) {
            ...
            if (!exceptions.isEmpty()) {
                writer.putContext("exceptions", writer.consumer(w -> {
                    ...
                    for (var e : exceptions) {
                        ...
                        var sparrowhawkError = symbolProvider.toSymbol(errorTarget);
                        var smithyError = symbolProvider.toSymbol(errorTarget).expectProperty(SMITHY_SYMBOL);
                        w.putContext("sparrowhawkError", sparrowhawkError);
                        w.putContext("smithyError", smithyError);
                        w.putContext("conditional", first ? "if" : "} else if");
                        w.putContext("sparrowhawkSetter", StringUtils.capitalize(e.getKey()));
                        first = false;
                        w.write("""
                            ${conditional:L} (exception instanceof ${smithyError:T} e) {
                                conv.set${sparrowhawkSetter:L}(${sparrowhawkError:T}.convertFrom(e));
                                unset = false;""");
                    }
                    w.write("}\n");
                }));
            ...
            writer.write("""
                ...
                public byte[] encodeEvent(${serializableStruct:T} event) {
                    return serialize(${sparrowhawkSymbol:T}.convertFrom((${smithySymbol:T}) event));
                }
                ...
                public byte[] encodeEventException(${serializableStruct:T} exception) {
                    ...
                    ${sparrowhawkSymbol:T} conv = new ${sparrowhawkSymbol:T}();
                    ${exceptions:C|}
                    if (unset) {
                        return null;
                    } else {
    ...
    private record OperationInfo(
        Symbol symbol, OperationShape operationShape, Symbol sparrowhawkInputSymbol, Symbol sparrowhawkOutputSymbol,

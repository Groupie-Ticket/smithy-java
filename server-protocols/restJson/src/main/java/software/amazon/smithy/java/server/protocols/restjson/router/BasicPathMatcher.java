...
class BasicPathMatcher implements Matcher {
    ...
    public BasicPathMatcher(CharSequence pattern) {
        this(new PathPattern(pattern));
    }

    BasicPathMatcher(PathPattern pattern) {
        if (pattern == null) {
    ...
    protected CharSequence decode(CharSequence encoded) {
        return urlEncoder.decodeUriComponent(encoded);
    }
}

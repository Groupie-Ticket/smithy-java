...
public abstract class NumberMap<T> implements KestrelObject {
    ...
    @Override
    public final void decodeFrom(KestrelDeserializer d) {
    ...
    private T[] readValues(KestrelDeserializer d, int n) {
        T[] values = newArray(n);
    ...
    protected abstract T decode(KestrelDeserializer d);

    @Override
    public final void encodeTo(KestrelSerializer s) {
        int size = size();
        if (size > 0) {
    ...
    protected abstract void writeValues(KestrelSerializer s, T[] values);
